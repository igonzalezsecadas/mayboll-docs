...
- [ ] `npm install -g mayboll-docs` (or local dev equivalent) exposes a `mayboll-docs` CLI with `init`, `dev`, and `build` subcommands
- [ ] `mayboll-docs init` creates the following in `./docs/`:
...
- [ ] `docs/content/index.md` serves as the homepage and is rendered with the same `DocLayout` as every other page
- [ ] `mayboll-docs dev` proxies to `astro dev` inside `./docs/`
- [ ] `mayboll-docs build` proxies to `astro build` inside `./docs/`
- [ ] The generated Astro project is a "normal" Astro project — users can `cd docs && npm install && npm run dev` directly without the CLI if they prefer
- [ ] Theme toggle persists preference to `localStorage` and applies it on page load
