...
- [ ] A remark/rehype plugin (or Astro integration) rewrites relative links ending in `.md` or `.mdx` to the correct output URL paths with trailing `/` (directory-style URLs)
...
- [ ] `mayboll-docs init` auto-detects the repository name and owner from `git remote get-url origin`:
...
- [ ] `mayboll-docs init` generates `.github/workflows/deploy.yml` at the repository root (creating `.github/workflows/` if needed)
...
- [ ] The generated workflow assumes a public GitHub Pages site (private Pages / auth is out of scope)
